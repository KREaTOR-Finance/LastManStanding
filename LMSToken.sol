...
/*
Last Man Standing - The only winner-takes-all token
in the Wild West of Defi

Telegram: https://t.me/lms_token
Twitter: https://twitter.com/lms_token
Reddit: https://www.reddit.com/r/LMS_token
*/
...
contract LastManStanding is IERC20, Ownable {
    ...
    string constant _name = "Last Man Standing";
    ...
    uint256 cakeGoldmineFee = 400; //4.0%
    uint256 marketingFee = 100; // 1.0%
    uint256 devFee = 50; // 0.5%
    uint256 metaFee = 50; // 0.5%
    uint256 totalFee = liquidityFee + cakeGoldmineFee + devFee + metaFee + marketingFee; // 8.0%
    ...
    function swapBack(uint256 amount) internal swapping {
        ...
        uint256 amountBNBMeta = (amountBNB * metaFee) / totalBNBFee;
        uint256 amountBNBGoldmine = (amountBNB * cakeGoldmineFee) / totalBNBFee;
        
        
        marketingFeeReceiver.transfer(amountBNBMarketing);
        devFeeReceiver.transfer(amountBNBDev);
        metaFeeReceiver.transfer(amountBNBMeta);
        goldmineReceiver.transfer(amountBNBGoldmine);
        ...
        emit FundsDistributed(amountBNBLiquidity, amountToLiquify, amountBNBGoldmine, amountBNBMarketing, amountBNBDev);
    }
    ...
    function setFees(uint256 _liquidityFee, uint256 _cakeGoldmineFee, uint256 _marketingFee, uint256 _addSellingFee) external onlyOwner {
        ...
        cakeGoldmineFee = _cakeGoldmineFee;
        ...
        emit UpdatedSettings('Fees', [Log('Total Buy Fee Percent', (totalFee) * 100 / feeDenominator), Log('Total Sell Fee Percent', (totalFee + addSellingFee) * 100 / feeDenominator), Log('Goldmine Percent', cakeGoldmineFee * 100 / feeDenominator)]);
    }
    ...
    event FundsDistributed(uint256 liquidityBNB, uint256 liquidityTokens, uint256 goldmineBNB, uint256 marketingBNB, uint256 devBNB);
